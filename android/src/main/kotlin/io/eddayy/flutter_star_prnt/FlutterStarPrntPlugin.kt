...
public class FlutterStarPrntPlugin : FlutterPlugin, MethodCallHandler {
  ...
  public fun checkStatus(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    try {
      ...
      val firmwareInformationMap :Map<String, String>  = port.getFirmwareInformation();
  ...
  public fun connect(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    try {
      ...
      starIoExtManager = StarIoExtManager( if (hasBarcodeReader != null && hasBarcodeReader) StarIoExtManager.Type.WithBarcodeReader else StarIoExtManager.Type.Standard, portName, portSettings, 10000, applicationContext);

      if (starIoExtManager != null) starIoExtManager.connect(object : IConnectionCallback {
  ...
  private fun appendCommands(builder: ICommandBuilder,printCommands: ArrayList<Map<String,Any>>?,context: Context) {
    var encoding :Charset = Charset.forName("US-ASCII");

    printCommands?.forEach { 
      if(it.containsKey("appendCharacterSpace"))  builder.appendCharacterSpace((it.get("appendCharacterSpace").toString()).toInt())
      else if(it.containsKey("appendEncoding"))  encoding = getEncoding(it.get("appendEncoding").toString())
      else if (it.containsKey("appendCodePage")) builder.appendCodePage(getCodePageType(it.get("appendCodePage").toString()))
      else if (it.containsKey("append")) builder.append(it.get("append").toString().toByteArray(encoding))
      else if (it.containsKey("appendRaw")) builder.append(it.get("appendRaw").toString().toByteArray(encoding))
      else if (it.containsKey("appendEmphasis")) builder.appendEmphasis(it.get("appendEmphasis").toString().toByteArray(encoding))
      else if (it.containsKey("enableEmphasis")) builder.appendEmphasis((it.get("enableEmphasis").toString()).toBoolean())
      else if (it.containsKey("appendInvert")) builder.appendInvert(it.get("appendInvert").toString().toByteArray(encoding))
      else if (it.containsKey("enableInvert")) builder.appendInvert((it.get("enableInvert").toString()).toBoolean())
      else if (it.containsKey("appendUnderline")) builder.appendUnderLine(it.get("appendUnderline").toString().toByteArray(encoding))
      else if (it.containsKey("enableUnderline")) builder.appendUnderLine((it.get("enableUnderline").toString()).toBoolean())
      else if (it.containsKey("appendInternational")) builder.appendInternational(getInternational(it.get("appendInternational").toString()))
      else if (it.containsKey("appendLineFeed")) builder.appendLineFeed((it.get("appendLineFeed") as Int))
      else if (it.containsKey("appendUnitFeed")) builder.appendUnitFeed((it.get("appendUnitFeed") as Int))
      else if (it.containsKey("appendLineSpace")) builder.appendLineSpace((it.get("appendLineSpace") as Int))
      else if (it.containsKey("appendFontStyle")) builder.appendFontStyle((getFontStyle(it.get("appendFontStyle") as String)))
      else if (it.containsKey("appendCutPaper")) builder.appendCutPaper(getCutPaperAction(it.get("appendCutPaper").toString()))
        ...
        if(it.containsKey("data")) builder.appendAlignment((it.get("data").toString().toByteArray(encoding)), getAlignment(it.get("appendAlignment").toString()));
        else builder.appendAlignment(getAlignment(it.get("appendAlignment").toString()));
      }
      else if (it.containsKey("appendHorizontalTabPosition")) builder.appendHorizontalTabPosition(it.get("appendHorizontalTabPosition") as IntArray) //TODO: test this in the future
      else if (it.containsKey("appendLogo")){
        if (it.containsKey("logoSize")) builder.appendLogo(getLogoSize(it.get("logoSize") as String) ,it.get("appendLogo") as Int)
        else builder.appendLogo(getLogoSize("Normal") ,it.get("appendLogo") as Int)
      }
      else if (it.containsKey("appendBarcode")){
        val barcodeSymbology :ICommandBuilder.BarcodeSymbology = if (it.containsKey("BarcodeSymbology")) getBarcodeSymbology(it.get("BarcodeSymbology").toString()) else getBarcodeSymbology("Code128")
        val barcodeWidth :ICommandBuilder.BarcodeWidth = if (it.containsKey("BarcodeWidth")) getBarcodeWidth(it.get("BarcodeWidth").toString()) else getBarcodeWidth("Mode2")
        val height :Int = if (it.containsKey("height")) (it.get("height").toString()).toInt() else 40
        val hri :Boolean = if (it.containsKey("hri")) (it.get("hri").toString()).toBoolean() else true

        if(it.containsKey("absolutePosition")){
            builder.appendBarcodeWithAbsolutePosition(it.get("appendBarcode").toString().toByteArray(encoding), barcodeSymbology, barcodeWidth, height, hri, it.get("absolutePosition") as Int);
        }else if(it.containsKey("alignment")){
            builder.appendBarcodeWithAlignment(it.get("appendBarcode").toString().toByteArray(encoding), barcodeSymbology, barcodeWidth, height, hri, getAlignment(it.get("alignment").toString()));
        }else builder.appendBarcode(it.get("appendBarcode").toString().toByteArray(encoding), barcodeSymbology, barcodeWidth, height, hri);
      } 
  ...
  private fun createBitmapFromText(printText :String, textSize :Float, printWidth :Int, typeface:Typeface): Bitmap{
    ...
    val staticLayout :android.text.StaticLayout = StaticLayout(printText, textPaint, printWidth, Layout.Alignment.ALIGN_NORMAL, 1.toFloat(), 0.toFloat(), false);

    // Create bitmap
    val bitmap: Bitmap = Bitmap.createBitmap(staticLayout.getWidth(), staticLayout.getHeight(), Bitmap.Config.ARGB_8888);
